...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                 'Jade' ),
			'home'                => __( 'Home',                    'Jade' ),
			'search'              => __( 'Search results for "%s"', 'Jade' ),
			'error_404'           => __( '404 Not Found',           'Jade' ),
			'paged'               => __( 'Page %d',                 'Jade' ),
			'archives'            => __( 'Archives',                'Jade' ),
			'archive_minute_hour' => __( 'g:i a',                   'Jade' ),
			'archive_minute'      => __( 'Minute %d',               'Jade' ),
			'archive_hour'        => __( 'g a',                     'Jade' ),
			'archive_day'         => __( 'd',                       'Jade' ),
			'archive_week'        => __( 'Week %d',                 'Jade' ),
			'archive_month'       => __( 'F',                       'Jade' ),
			'archive_year'        => __( 'Y',                       'Jade' ),
		//	'edit'                => __( 'Edit',                    'Jade' ), // @todo Implement edit link
		);
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'Jade' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'Jade' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'Jade' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
